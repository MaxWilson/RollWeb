...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,jQuery,UI,Next,Var,ListModel1,Roll,Client,Doc,View,String,Roller,Attr,ListModel,List,Resolver,Seq,Operators,Random;
 Runtime.Define(Global,{
  Roll:{
   Client:{
    Main:Runtime.Field(function()
    {
     var nextRoll,x,ListContainer,Roll1;
     jQuery("#main").empty();
     nextRoll=Var.Create("");
     x=ListModel1.View(Client.Rolls());
     ListContainer=Doc.Convert(function(tupledArg)
     {
      var result,LastRoll,Result;
      result=tupledArg[1];
      LastRoll=View.Const(tupledArg[0]);
      Result=View.Const(String(result));
      return Doc.Concat([Doc.Element("li",[],[Doc.TextView(LastRoll),Doc.TextNode(" "),Doc.TextView(Result)])]);
     },x);
     Roll1=function()
     {
      Client.Rolls().Add(["2d6",Roller.Instance().Resolve([2,6])]);
      return Var.Set(nextRoll,"");
     };
     return Doc.RunById("main",Doc.Concat([Doc.Element("ul",[],[ListContainer]),Doc.Element("div",[],[Doc.Input([Attr.Create("placeholder","Roll specification, e.g. 2d6")],nextRoll),Doc.Element("button",[Attr.Handler("click",Roll1)],[Doc.TextNode("Roll")]),Doc.Element("div",[],[Doc.TextNode("About to roll: "),Doc.TextView(View.FromVar(nextRoll))])])]));
    }),
    Rolls:Runtime.Field(function()
    {
     return ListModel.FromSeq(List.ofArray([["2d6+3",7]]));
    })
   }
  },
  Roller:{
 ...
 Runtime.OnInit(function()
 {
  ...
  Roll=Runtime.Safe(Global.Roll);
  Client=Runtime.Safe(Roll.Client);
  Doc=Runtime.Safe(Next.Doc);
  View=Runtime.Safe(Next.View);
  String=Runtime.Safe(Global.String);
  Roller=Runtime.Safe(Global.Roller);
  Attr=Runtime.Safe(Next.Attr);
  ListModel=Runtime.Safe(Next.ListModel);
  List=Runtime.Safe(Global.WebSharper.List);
  Resolver=Runtime.Safe(Roller.Resolver);
  Seq=Runtime.Safe(Global.WebSharper.Seq);
  Operators=Runtime.Safe(Global.WebSharper.Operators);

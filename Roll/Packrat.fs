...
type Head<'a> = { headRule: Rule<'a>; involved: Set<string> ref; eval: Set<string> ref}
...
type ParserContext<'a>() =
    ...
    member this.PopLR() =
        if this.LRStack.Value.IsSome then
            this.LRStack := !this.LRStack.Value.Value.rest
...
let memoize (ctx: ParserContext<'a>) =
    ...
    let setupLeftRecursion((name, rule):Rule<'a>, lr: LR<'a>) =
        let LRHead = 
            match (!lr.head) with
            | None ->
                let h = { headRule = (name, rule); involved = ref Set.empty; eval = ref Set.empty }
                lr.head := Some(h)
                h
            | Some(h) -> h
